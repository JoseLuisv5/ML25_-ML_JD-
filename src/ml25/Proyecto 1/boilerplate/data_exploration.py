...
def read_csv(name: str): return pd.read_csv(DATA_DIR / f"{name}.csv")
...
if __name__ == "__main__":
    ...
    train["customer_age_range"] = _age_range(train["customer_age_years"])
    ...
    if "customer_gender" in train.columns: train["customer_gender"].value_counts().plot(kind="bar", ax=ax[2], title="Género")
    else: ax[2].set_visible(False)
    if "purchase_device" in train.columns: train["purchase_device"].value_counts().plot(kind="bar", ax=ax[3], title="Medio")
    else: ax[3].set_visible(False)

    train["item_color"].value_counts().plot(kind="bar", ax=ax[4], title="Color")
    adjs = _adj_counts(train["item_title"] if "item_title" in train.columns else None).head(10)
    if not adjs.empty: adjs.plot(kind="bar", ax=ax[5], title="Adjetivos")
    else: ax[5].set_visible(False)

    train["item_category"].value_counts().head(8).plot(kind="pie", ax=ax[6], autopct="%1.0f%%", ylabel="", title="Categoría")
    train["item_category"].value_counts().head(10).plot(kind="bar", ax=ax[7], title="Categoría (bar)")
    train["customer_age_range"].value_counts().sort_index().plot(kind="bar", ax=ax[8], title="Rangos de edad")

    if "item_price" in train.columns and train["item_price"].notna().any(): train["item_price"].dropna().plot(kind="hist", bins=20, ax=ax[9], title="Precio")
    else: ax[9].set_visible(False)
    if "item_avg_rating" in train.columns and train["item_avg_rating"].notna().any(): train["item_avg_rating"].dropna().plot(kind="hist", bins=10, ax=ax[10], title="Rating")
    else: ax[10].set_visible(False)
    ...
    for j, b in enumerate(buckets):
        ...
        if len(sub) > 0 and "item_category" in sub.columns:
            sub["item_category"].value_counts().head(4).plot(kind="bar", ax=ax[idx], title=f"Top ropa {b}")
        else:
